   ...
   <Options.Tips>
	...
	Extras.Classes.Navigation = new Class({
		...
		onMouseWheel: function(e, win, scroll) {
			...
			var val = this.config.zooming / 50,
			ans = 1 + scroll * val;
   ...
   Contains rendering and other type of primitives for simple shapes.
	...
	var NodeHelper = {
	   ...
	   Object: NodeHelper.circle
		...
		/*
                Object: NodeHelper.host
                 */
		'host': {
			'render': function(type, node, canvas) {
				var ctx = canvas.getCtx();
                                if (ctx){
                                        var img = new Image();
                                        img.src = 'images/host1.gif';
                                        node.pos.setc(node.getData('x'), node.getData('y'));
                                        //node.startPos.setc(node.getData('x'), node.getData('y'));
                                        //node.endPos.setc(node.getData('x'), node.getData('y'));
                                        var pos = node.getPos();
                                        img.onload = function(){
						ctx.drawImage(img, pos.x-40, pos.y-72, 90, 73);
                                        };
                                }
			},
			'contains': function(npos, pos, dim) {
                                return NodeHelper.square.contains(npos, pos, dim);
                        }
		},
		/*
                Object: NodeHelper.of-switch
                 */
                'of_switch': {
                        'render': function(type, node, canvas) {
                                var ctx = canvas.getCtx();
                                if (ctx){
                                        ...
                                        var pos = node.getPos();
                                        img.onload = function(){
                                                ctx.drawImage(img, pos.x-60, pos.y+10, 120, 28);
                                        };
                                }
                        },
                        'contains': function(npos, pos){
                                return NodeHelper.rectangle.contains(npos, pos, 120, 28);
                        }
                },
		/*
                Object: NodeHelper.controller
                 */
                'controller': {
                        'render': function(type, node, canvas) {
                                var ctx = canvas.getCtx();
                                if (ctx){
                                        ...
                                        var pos = node.getPos();
                                        img.onload = function(){
						ctx.drawImage(img, pos.x-40, pos.y-99, 71, 100);
                                        };
                                }
                        },
                        'contains': function(npos, pos) {
                                return NodeHelper.rectangle.contains(npos, pos, 71, 100);
                        }
                },
		/*
                Object: NodeHelper.rf-server
                 */
                'rf_server': {
                        'render': function(type, node, canvas) {
                                var ctx = canvas.getCtx();
...
//	Some util shared layout functions are defined here.
	...
	Layouts.ForceDirected = new Class({
		...
		computePositionStep: function(property, opt) {
			...
			tableMatches = new Array(this.graph.nodes.length);
	...
	$jit.ForceDirected = new Class( {
		...
		printInfo: function(node) {
			if(node) {
				...
				if(switchData) {	
					...
					list.push("sw_desc = " + switchData[2]);
					list.push("serial_num = " + switchData[3]);
					list.push("dp_desc = " + switchData[4]);
					html = html + list.join("</li><li>") + "</li></ul>";
				
				
				//ofp_aggregate_stats
				switchData = node.getData('ofp_aggregate_stats');
				if(switchData) {
					html = html + "<b>ofp_agregate_stats:</b><ul><li>", list = [];
					list.push("packet_count = " + switchData[0]);
					list.push("byte_count = " + switchData[1]);
					list.push("flow_count = " + switchData[2]);
					html = html + list.join("</li><li>") + "</li></ul>";
				}
				
				//ofp_table_stats
				switchData = node.getData('ofp_table_stats');
				html = html + "<b>ofp_table_stats:</b><ul><li>", list = [];
				if(switchData) {
					list.push("table_id = " + switchData[0][0]);
					list.push("name = " + switchData[0][1]);
					list.push("active_count = " + switchData[0][2]);
					list.push("lookup_count = " + switchData[0][3] + "</li></ul><ul>");
					//list.push("table_id = " + switchData[1][0]);
					//list.push("name = " + switchData[1][1]);
					//list.push("active_count = " + switchData[1][2]);
					//list.push("lookup_count = " + switchData[1][3]);
					html = html + list.join("</li><li>") + "</li></ul>";
				}
				...
				html = "<div id='bottom-table'>" +
				    ...
				    "<tr class='table-row-header'>" +
					    ...
					    "<td style='width: 510px;'>Actions</td>" +
					    "<td style='width: 60px;'>Packets</td>" +
					    "<td style='width: 60px;'>Bytes</td>" +
				    "</tr>";
				var matches = "";
				for(var i in flows) {
					var f = flows[i];
					...
					if(tableMatches[node.id]!=undefined && tableMatches[node.id].search(f.ofp_match)!=-1) {
						...
						if(f.ofp_match in contMatch && contMatch[f.ofp_match]>0) {
							...
							html = html +
                                                        	"<tr class='table-row-content-not-so-new'> " +
        	                                                "<td style='border-right:1px dashed; width: 38px;'>" + f.flow + "</td>";

                	                                html = html + "<td style='border-right:1px dashed; width: 250px;'>" + f.ofp_match + "</td>";

                        	                        html = html + "<td style='border-right:1px dashed; width: 510px;'>" + actions + "</td>" +
                                	                        "<td style='border-right:1px dashed; width: 60px;'>" + f.packet_count + "</td>" +
                                        	                "<td style='width: 60px;'>" + f.byte_count + "</td>" +
                                                	        "</tr>";
						// fluxo estah na flow table ha tempo
							...
							if(flowsActions && flowsActions[f.ofp_match]!=undefined && flowsActions[f.ofp_match] != actions) {
									...
									html = html +
	                                                                "<tr class='table-row-content-action-changed'> " +
        	                                                        "<td style='border-right:1px dashed; width: 38px;'>" + f.flow + "</td>";
								...
								if(contActions[f.ofp_match] && contActions[f.ofp_match]>0) {
									...
									html = html +
										"<tr class='table-row-content-not-selected' " +
	                	                        	        	"onmouseover=this.className='table-row-content-selected'; "+
		                	        	                        "onmouseout=this.className='table-row-content-not-selected';>" +
										"<td style='border-right:1px dashed; width: 38px;'>" + f.flow + "</td>";
					...
					// fluxo acaba de entrar na flow table
						...
						html = html + "<tr class='table-row-content-new'>" +
							"<td style='border-right:1px dashed; width: 38px;'>" + f.flow + "</td>";
				...
				$jit.id('bottom-details').innerHTML = html;}
			}
		},
		...
		updateJSON: function(json, onComplete) {
			...
			this.graph = this.construct(json);
			//var auxGraph = this.graph;
			var that = this;
			//var nodesPosition = new Array();

			this.graph.eachNode(function(node){
				//aux = new Array(oldGraph.getNode(node.id).getPos().x , oldGraph.getNode(node.id).getPos().y);
		...
		loadStats: function(json, onComplete) {
			if(this.busy) return;
			this.busy = true;
			var statsGraph = this.construct(json);
			
			this.graph.eachNode(function(node) {
				if(statsGraph.getNode(node.id)) {
					node.setData('dp_id', statsGraph.getNode(node.id).getData('dp_id'));
					node.setData('flows', statsGraph.getNode(node.id).getData('flows'));
					node.setData('ofp_desc_stats', statsGraph.getNode(node.id).getData('ofp_desc_stats'));
					node.setData('ofp_aggregate_stats', statsGraph.getNode(node.id).getData('ofp_aggr_stats'));
					node.setData('ofp_table_stats', statsGraph.getNode(node.id).getData('ofp_table_stats'));
					node.setData('ifconfig', statsGraph.getNode(node.id).getData('ifconfig'));
				}
			}),
	...
	(function(ForceDirected) {
		...
		ForceDirected.Plot.NodeTypes = new Class({
			...
			'host': {
                                'render': function(node, canvas){
                                        this.nodeHelper.host.render('fill', node, canvas);
                                },
                                'contains': function(node, pos){
                                        var npos = node.pos.getc(true),
                                        dim = node.getData('dim');
                                        return this.nodeHelper.host.contains(npos, pos, dim);
                                }
                        },
			'of-switch': {
				'render': function(node, canvas){
					this.nodeHelper.of_switch.render('fill', node, canvas);
				},
				'contains': function(node, pos) {
					var npos = node.pos.getc(true); 
					return this.nodeHelper.of_switch.contains(npos, pos);
				}
			},
			'controller': {
				'render': function(node, canvas){
					this.nodeHelper.controller.render('fill', node, canvas);
				},
				'contains': function(node, pos) {
					var npos = node.pos.getc(true); 
					return this.nodeHelper.controller.contains(npos, pos);
				}
			},
			'rf-server': {
                                'render': function(node, canvas){
					this.nodeHelper.rf_server.render('fill', node, canvas);
                                },
                                'contains': function(node, pos) {
                                        var npos = node.pos.getc(true);
                                        return this.nodeHelper.rf_server.contains(npos, pos);
                                }
                        },
			'star': {
				'render': function(node, canvas){
		...
		});	// end of ForceDirected.Plot.NodeTypes
